...
- `apps/` holds the externally exported applications, assembled from Client / Host mixtures.
    ...
    - `apps/desktop` reuses the same client graph over typed Electron IPC, without opening an HTTP or WebSocket listener.
...
1. **Pick a carrier**: browser same-origin HTTP/WebSocket, process-local fetch/SSE, or typed IPC such as `apps/desktop` (see the subclass table below).
...
| `ElectronApiClient` | `apps/desktop` | authenticated Electron IPC round trips plus dedicated stream channels | preserves the four envelopes, cancellation, interaction responses, mux/host streams, and generic Connection RPC without a listening socket |
