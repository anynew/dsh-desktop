...
export function apply(ctx: Context): void {
  ...
  const api: IApiClient = fixtureClient ?? desktop?.api ?? transport?.createApiClient() ?? new WebApiClient()
  const rpc = fixtureClient?.rpc ?? desktop?.rpc ?? createWebConnectionRpc(transport?.fetch)
