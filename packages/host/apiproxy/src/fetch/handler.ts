...
export function toFetchHandler(api: ApiProxy): { fetch: typeof fetch } {
  return {
    ...
    async fetch(input: RequestInfo | URL, init?: RequestInit): Promise<Response> {
      ...
      if (path === '/api/respond') {
        return Response.json(await dispatchClientResponse(api, body))
      }
      ...
      try {
        return Response.json(await dispatchRoutedClientRequest(api, method, body, req.signal))
      } catch (error: unknown) {
        // A valid request reaching a throwing implementation is a carrier failure.
